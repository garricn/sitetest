 ...
 */
export async function saveBaseline(captureName, sourceDir, baselinesDir, variant) {
  const dest = variant
    ? resolve(baselinesDir, captureName, variant)
    : resolve(baselinesDir, captureName);
 ...
 */
export async function baselineExists(captureName, baselinesDir, variant) {
  try {
    const dir = variant
      ? resolve(baselinesDir, captureName, variant)
      : resolve(baselinesDir, captureName);
 ...
 * @returns {string}
 */
export function baselinePath(captureName, baselinesDir, variant) {
  return variant
    ? resolve(baselinesDir, captureName, variant)
    : resolve(baselinesDir, captureName);
}
