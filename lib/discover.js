...
export async function discover(opts) {
  ...
  for (const pageDir of pageDirs) {
    ...
    // Read meta.json for URL and capture duration
    let site = opts.site || "";
    let pagePath = "/";
    try {
      ...
      if (typeof meta.duration_ms === "number") {
        totalCaptureDuration += meta.duration_ms;
        pagesWithDuration++;
      }
    } catch {
    ...
    pending.push({ runbook, runbookPath, elements: elements.length, testable: testable.length });
  }

  // Write all runbook files with consistent elapsed time
  const discoveryTime = ((Date.now() - startTime) / 1000).toFixed(1);
  let timingPart;
  if (pagesWithDuration === 0) {
    timingPart = `discovery: ${discoveryTime}s (capture time not included)`;
  } else {
    const captureSec = (totalCaptureDuration / 1000).toFixed(1);
    const prefix = pagesWithDuration < pending.length ? "~" : "";
    timingPart = `capture: ${prefix}${captureSec}s, discovery: ${discoveryTime}s`;
  }
  const header = `# Generated by sitetest.discover() — ${pageDirs.length} pages, ${timingPart}. Do not edit by hand.\n`;
  for (const { runbook, runbookPath, elements, testable } of pending) {
    await writeFile(runbookPath, header + yaml.dump(runbook, { lineWidth: 120, noRefs: true }));
    results.push({ name: runbook.name, path: runbookPath, elements, testable });
  }
