...
export function printResult(result) {
  ...
  for (const step of result.steps) {
    ...
    const variantTag = step.variant ? `${DIM}[${step.variant}]${RESET} ` : "";
    console.log(`    ${icon} ${variantTag}${step.label} ${duration}`);
  ...
  if (result.videoPath) {
    console.log(`  ${DIM}Video: ${result.videoPath}${RESET}`);
  }
  console.log();
}
...
export function formatJunit(results) {
  ...
  for (const result of list) {
    ...
    const properties = result.videoPath
      ? `\n    <properties>\n      <property name="videoPath" value="${esc(result.videoPath)}" />\n    </properties>`
      : "";

    suites.push(
      `  <testsuite name="${esc(result.runbook)}" tests="${tests}" failures="${failures}" time="${time}">${properties}\n${testcases}\n  </testsuite>`
    );
  }
