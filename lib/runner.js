...
export async function runRunbook(opts) {
  ...
  try {
    ...
    const session = await createCaptureSession(browser, defaultViewport, sessionOpts);
    ...
    // Build variant list — default to single no-variant run
    const variants = runbook.variants
      ? runbook.variants.map((v) => ({
          name: v.name || `${v.viewport.width}x${v.viewport.height}${v.colorScheme ? `-${v.colorScheme}` : ""}`,
          viewport: v.viewport,
          colorScheme: v.colorScheme || null,
        }))
      : [{ name: null, viewport: defaultViewport, colorScheme: null }];
    ...
    for (const variant of variants) {
      ...
      for (const step of runbook.steps) {
        ...
        // Tag results with variant name for reporting
        const tagResult = (r) => {
          if (variant.name) r.variant = variant.name;
          return r;
        };
        ...
        if (Array.isArray(result)) {
          ...
          stepResults.push(tagResult(result));
