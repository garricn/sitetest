...
async function executeStepInner(type, def, page, ctx) {
  switch (type) {
    ...
    case "capture": {
      // Phase 2 — stub that records the capture was requested
      ctx.captures = ctx.captures || [];
      ctx.captures.push({ name: def.name || "unnamed", baseline: !!def.baseline, diff: !!def.diff });
      return;
    }
