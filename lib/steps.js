...
import { capturePage } from "sitecap/capture";
...
export function parseStep(rawStep) {
  ...
  const metaKeys = new Set(["timeout", "file"]);
  const stepKeys = keys.filter((k) => !metaKeys.has(k));
  if (stepKeys.length !== 1) {
    throw new Error(`Invalid step: expected exactly one step type, got: ${stepKeys.join(", ")}`);
  }
  const type = stepKeys[0];
  let def = rawStep[type];
  const file = rawStep.file;
  // When file is present and def is a string, promote to object form
  if (file !== undefined && typeof def === "string") {
    def = { action: def, file };
  } else if (file !== undefined && typeof def === "object" && def !== null) {
    def = { ...def, file };
  }
  return { type, def, timeout: rawStep.timeout };
}
...
export async function executeStep(rawStep, page, ctx) {
  ...
  try {
    ...
    // Run steps return an array of sub-step results — pass through directly
    if (type === "run" && Array.isArray(stepResult)) {
      return stepResult;
    }
...
async function executeStepInner(type, def, page, ctx) {
  switch (type) {
    ...
    case "capture": {
      ...
      // Capture to a temp directory (nested under variant if present)
      const variant = ctx.variant || null;
      const captureOutDir = variant
        ? resolve(ctx.capturesDir, captureName, variant)
        : resolve(ctx.capturesDir, captureName);
      ...
      if (isBaseline) {
        await saveBaseline(captureName, captureOutDir, ctx.baselinesDir, variant);
        return;
      }

      if (isDiff) {
        const hasBaseline = await baselineExists(captureName, ctx.baselinesDir, variant);
        if (!hasBaseline) {
          throw new Error(`Capture "${captureName}": no baseline found. Run with baseline: true first.`);
        }
        const baseDir = baselinePath(captureName, ctx.baselinesDir, variant);
    ...
    case "run": {
      const flowPath = await resolveFlowPath(def, ctx.runbookDir);
      const content = await readFile(flowPath, "utf-8");
      const flow = yaml.load(content);
      if (!flow || !Array.isArray(flow.steps)) {
        throw new Error(`Invalid sub-flow: ${flowPath} — missing "steps" array`);
      }
      // Execute sub-flow steps inline, return array of results
      const results = [];
      for (const subStep of flow.steps) {
        ctx.previousUrl = page.url();
        const r = await executeStep(subStep, page, ctx);
        results.push(r);
    ...
    case "cookies": {
      ...
      const file = typeof def === "object" ? def.file : undefined;
      if (action === "save") {
        if (!file) throw new Error('cookies save requires a "file" key');
        const filePath = resolve(ctx.runbookDir, file);
        await mkdir(dirname(filePath), { recursive: true });
        const cookies = await page.context().cookies();
        await writeFile(filePath, JSON.stringify(cookies, null, 2));
      } else if (action === "load") {
        if (!file) throw new Error('cookies load requires a "file" key');
        const filePath = resolve(ctx.runbookDir, file);
        const cookies = JSON.parse(await readFile(filePath, "utf-8"));
...
async function resolveFlowPath(flowRef, runbookDir) {
  const base = join(runbookDir, flowRef);
  // Try .yaml, .yml, then as-is
  for (const candidate of [`${base}.yaml`, `${base}.yml`, base]) {
    try {
  ...
  throw new Error(`Sub-flow not found: ${flowRef} (searched in ${runbookDir})`);
}
...
function stepLabel(type, def) {
  switch (type) {
    ...
    case "cookies": {
      const action = typeof def === "string" ? def : def.action;
      const file = typeof def === "object" ? def.file : undefined;
      return file ? `cookies ${action} → ${file}` : `cookies ${action}`;
    }
    default:
      return type;
  }
